...
/*
  ...
  Todos los cromosomas se generarán aleatoriamente.
*/

void generate_Poblationvector(vector<Features> &Poblacion, int genes, int cromosomas)
{
	Features C;
...
void populationEvaluation(BaseDatos &train, BaseDatos &testVal, vector<Features> &Pactual, Features &features_mejor, Features &features_peor, int &evaluaciones)
{
	...
	// Buscar la mejor solución de la población
	if(!mejor)
	{ 
		for(int i=0; i<Pactual.size(); i++)
		{
	...
	// Buscar la mejor solución de la población en caso de que no se haya evaluado ninguna 
	if(!peor)
	{
		for(int i=0; i<Pactual.size(); i++)
		{
...
*/
void selection(BaseDatos &train, BaseDatos &testVal, vector<Features> &Pactual, vector<Features> &Ppadres, int k_torneo, int cromosomas, int agGeneracional=0)
{
...
*/
void crossing(vector<Features> &Ppadres, vector<Features> &Pintermedia, double prob_c, int genes, int cromosomas)
{
	int parejas;
...
*/
void uniformMutation(vector<Features> &Pintermedia, vector<Features> &Phijos, double prob_g, int genes, int cromosomas, int estacional)
{
...
/*
  ...
  población. 
*/
void elitismReplacement(BaseDatos &train, BaseDatos &testVal, vector<Features> &Phijos, vector<Features> &Pactual, int &evaluaciones)
{
	int peor_hijo=-1;
	Features padre, hijo;
	...
	for(int i=0; i<Pactual.size(); i++)
	{
		if(Pactual[i].fobj_mejor)
			padre = Pactual[i];
		// Actualizar aquellos hijos sin evaluar
		if(!(Phijos[i].evaluada))
		{
			train.EvaluationFunctionFeatures(Phijos[i], testVal);
			Phijos[i].evaluada = true;
			evaluaciones++;
		}
...
void AGE(BaseDatos &train, BaseDatos &testVal, vector<int> &sol_mejor, double &prob_c, double &prob_g, int k_torneo)
{
	...
	do
	{
		...
		// Selección de padres
		//start = clock();
		selection(train, testVal, Pactual, Ppadres, k_torneo, CROMOSOMAS);
		...
		// Selección de padres
		//start = clock();
		uniformMutation(Pintermedia, Phijos, prob_g, genes, CROMOSOMAS, 1);
		//duration = ( clock() - start ) / (double) CLOCKS_PER_SEC;
